...
class test_parameters(object):
    ...
    parameters = {
              ...
              ('OpticalPumpingAux','aux_op_enable'):False,
              ('OpticalPumpingAux','aux_optical_pumping_amplitude_729'):WithUnit(-11.0, 'dBm'),
              ('OpticalPumpingAux','aux_optical_frequency_729'):WithUnit(0.0, 'MHz'),
              ...
              ('LocalStarkShift','amplitude'):WithUnit(-11.0, 'dBm'),
              ('LocalStarkShift', 'detuning'):WithUnit(0.0, 'MHz'),
              }
...
#            print 'parameter not lisfdted among requirments', collection, param
#    for key in d.keys():
#        a = TreeDict()
#        a.update(d)
#        a.pop(key)
...
#     print sorted(dds_result.difference(dds_set))
    ...
    # print cxn.pulser.get_readout_counts()
    # readout = cxn.pulser.get_readout_counts().asarray
    # print "Collected Readouts {}".format(readout)
    # print np.asarray(cxn.pulser.get_channels())
    # print np.asarray(ttl)
    print dds
    # print np.asarray(dds)
    channels = np.asarray(cxn.pulser.get_channels())#.asarray
    sp = SequencePlotter(np.asarray(ttl), dds, channels)
    sp.makePlot()