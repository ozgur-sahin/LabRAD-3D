...
class linear_fit():
    ...
    def fit(self, t, nbar, nbarerror):
        model = lambda x, m, c: m*x+c
        # weighting=1/nbarerror**2
        # print weighting
        popt, pcov = curve_fit(model, t, nbar, p0 = None, sigma = nbarerror,
            absolute_sigma = True)
        fit_errors = np.sqrt(np.diag(pcov))

        return (popt[0], fit_errors[0], popt[1], fit_errors[1])

class gauss_fit():

    def guess(self, f, p, force_guess = False):
         # center, amplitude, FWHM guesses
        max_index = np.where(p == p.max())[0][0]
        fmax=f[max_index]
        FWHM = 0.01 # in kHz
        return np.array([ p.max(), fmax, FWHM])

    def fit(self, f, p, no_of_repeats):
        model = lambda x, a, c0, FWHM: a*np.exp(-(x-c0)**2*4*np.log(2)/FWHM**2)
        force_guess = False
        # if return_all_params: force_guess = True
        # guess = self.guess(f, p, force_guess)
        bounds = ((0, f.min(), 0.003),(1, f.max(), 0.03))
        # perr = [np.sqrt(x*(1-x)/no_of_repeats) for x in p]
        # for index in range(0,len(p)):
        #     if perr[index] == 0:
        #         perr[index] = np.sqrt((1-1/no_of_repeats)/no_of_repeats**2)
        # if return_all_params: force_guess = True
        guess = self.guess(f, p, force_guess)
        popt, pcov = curve_fit(model, f, p, p0 = guess,  bounds = bounds)# sigma = perr,
        fit_errors = np.sqrt(np.diag(pcov))
        # print popt
        # print fit_errors
        # y_model = [popt[0]*np.exp(-(x-popt[1])**2*4*np.log(2)/popt[2]**2) for x in f]
        return (popt[0], popt[1], popt[2], fit_errors[0], 
            fit_errors[1], fit_errors[2])
