...
class peak_fitter():
    ...
    def fit_with_errors(self, f, p, return_all_params = False):
        model = lambda x, c0, a, w: a*np.exp(-(x - c0)**2/w**2)
        force_guess = False
        if return_all_params: force_guess = True
        guess = self.guess(f, p, force_guess)
        popt, copt = curve_fit(model, f, p, p0=guess)#, bounds=([-np.inf,-np.inf,0.,-np.inf],[np.inf,np.inf,np.inf,np.inf]))
        perr = np.sqrt(np.diag(copt))
        if return_all_params:
            return popt[0], popt[1], popt[2], perr[0], perr[1], perr[2] # center value, amplitude, width
        else:
            return popt[0] # return only the center value
