...
class calibrate_temperature(experiment):
    ...
    def run(self, cxn, context):
        ...
        try:
            ...
            # fit_center, rsb_ex, fit_width, fit_center_err, rsb_ex_err, fit_width_err = self.fitter.fit_with_errors(fr, ex, return_all_params = True)
            # if rsb_ex < 0:
            #     rsb_ex = np.max(ex)
            # elif rsb_ex > np.max(ex):
            #     rsb_ex = np.max(ex)
        except:
            rsb_ex = np.max(ex)
            # rsb_ex_err = 0.
        ...
        try:
            ...
            # fit_center, bsb_ex, fit_width, fit_center_err, bsb_ex_err, fit_width_err = self.fitter.fit_with_errors(fr, ex, return_all_params = True)
            # if bsb_ex < 0:
            #     bsb_ex = np.max(ex)
            # elif bsb_ex > np.max(ex):
            #     bsb_ex = np.max(ex)
        except:
            bsb_ex = np.max(ex)
            # bsb_ex_err = 0.
            
        return (rsb_ex, bsb_ex)#, rsb_ex_err, bsb_ex_err)
