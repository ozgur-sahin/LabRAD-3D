...
class calibrate_temperature(experiment):
    ...
    def run(self, cxn, context):
        ...
        try:
            (rsb_ex, fit_center, fit_width, rsb_ex_err, center_error,
                width_error) = self.fitter.fit(fr, ex, no_of_repeats)
        except:
            rsb_ex = np.max(ex)
            rsb_ex_err = 1
        ...
        # take the maximum of the line excitation
        try:        
            (bsb_ex, fit_center, fit_width, bsb_ex_err, center_error,
                width_error) = self.fitter.fit(fr, ex, no_of_repeats)
        except:
            bsb_ex = np.max(ex)
            bsb_ex_err = 1
            
        return (rsb_ex, rsb_ex_err, bsb_ex, bsb_ex_err)
