...
class calibrate_heating_rates(experiment):
    ...
    def run(self, cxn, context):
        ...
        for i,heat_time in enumerate(self.scan):
            ...
            # (rsb_ex, bsb_ex, rsb_ex_err, bsb_ex_err) = self.calibrate_temp.run(cxn, context)
            (rsb_ex, bsb_ex) = self.calibrate_temp.run(cxn, context)

            fac = rsb_ex/bsb_ex
            nbar = fac/(1.0-fac)
            # r_err_percent = rsb_ex_err/rsb_ex
            # b_err_percent = bsb_ex_err/bsb_ex
            # fac_err_percent = np.sqrt(np.power(r_err_percent,2) + np.power(b_err_percent,2))/fac
            # nbar_err_percent = np.sqrt(np.power(fac_err_percent,2) + np.power(fac_err_percent,2))/nbar
            # nbar_err = nbar*np.sqrt(np.power(rsb_ex_err/rsb_ex,2) + (np.power(bsb_ex_err,2) + np.power(rsb_ex_err,2)) / np.power(bsb_ex-rsb_ex,2) )
            # print nbar
            # print nbar_err

            submission = [heat_time['us']]
            submission.extend([nbar])
            # submission.extend([nbar, nbar_err])
            #print nbar
