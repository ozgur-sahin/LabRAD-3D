...
class point(pulse_sequence):
    ...
    def sequence(self):
        ...
        freq_729 = self.calc_freq_from_array(e.line_selection, e.sideband_selection)
...
class Scan3points(pulse_sequence):
    ...
    def run_in_loop(cls,cxn, parameters_dict, data, x):
        ...
        if dac.get_multipole_values() != multipoles_new:
            dac.set_multipole_values(multipoles_new, int(dac.get_position()), dac.get_center_voltage())
...
class ScanMicromotion(pulse_sequence):
    ...
    def run_finally(cls,cxn, parameters_dict, data, x):
        dac = cxn.dac_server
        dac.set_multipole_values(multipoles_initial, int(dac.get_position()), dac.get_center_voltage())