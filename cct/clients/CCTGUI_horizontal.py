...
class CCTGUI(QtGui.QMainWindow):
    ...
    def makeLightWidget(self, reactor, cxn):        
        ...
        gridLayout.addWidget(DAC_Control(reactor), 1, 0)

        DDS_tab = QtGui.QTabWidget()
        DDS_tab.addTab(DDS_CONTROL(reactor, 'Pulser2'), 'Pulser2')
        DDS_tab.addTab(DDS_CONTROL(reactor, 'Pulser'), 'Pulser1')

        gridLayout.addWidget(DDS_tab,0, 0)
        gridLayout.addWidget(actions_widget(reactor, cxn), 1, 1)
    ...
    def makeVoltageWidget(self, reactor):        
        from DAC_CONTROL_HORIZONTAL import DAC_Control
